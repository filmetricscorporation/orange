...
class SystemUserService extends BaseService{
    ...
    public function saveSystemUser(SystemUser $systemUser,$changePassword = false){
        ...
        try {
            if ($changePassword) {
                ...
                // 20160310 - update user_password_asp field
                $userPassword = $systemUser->getUserPassword();
                //$systemUser->setUserPassword($this->hashPassword($systemUser->getUserPassword()));
                $systemUser->setUserPassword($this->hashPassword($userPassword));                
                $systemUser->setUserPasswordAsp(md5($userPassword));
            }
   ...
   /**
    ...
    // 20160310 - this is the original updatePassword function of OrangeHRM
     //public function updatePassword($userId, $password) {
     //    return $this->getSystemUserDao()->updatePassword($userId, $this->hashPassword($password));
     //}
     
     public function updatePassword($userId, $password, $originalPassword = NULL) {
         return $this->getSystemUserDao()->updatePassword($userId, $this->hashPassword($password), $password);
     }
