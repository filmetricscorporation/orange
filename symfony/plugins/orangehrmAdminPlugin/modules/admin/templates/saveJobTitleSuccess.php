        ...
        <form name="frmSavejobTitle" id="frmSavejobTitle" method="post" action="<?php echo url_for('admin/saveJobTitle?jobTitleId=' . $form->jobTitleId); ?>" enctype="multipart/form-data" class="<?php echo empty($form->jobTitleId)?'':'clickToEditForm'; ?>">
            ...
            <fieldset>
                ...
                <ol>
                    ...
                    <?php // 20160318 - add approval-signatory in form?>
                    <li>
                        <?php echo $form['approvalSignatory']->renderLabel(__('Approval Signatory') . ' <em>*</em>'); ?>
                        <?php echo $form['approvalSignatory']->render(array("maxlength" => 2, "placeholder" => "2")); ?>
                    </li>
...
<script type="text/javascript">
    ...
    var lang_ApprovalSignatoryRequired = '<?php echo __(ValidationMessages::REQUIRED); ?>';

    if (($('#jobTitle_approvalSignatory').val() < 0) || ($('#jobTitle_approvalSignatory').val() == '')) {
        $('#jobTitle_approvalSignatory').val(-1);
    } 

    // 20160318 - restrict input to numbers
    $('#jobTitle_approvalSignatory').keyup(function(event) {
