...
class SystemUserDao extends BaseDao {
    ...
    // 20160310 original updatePassword function of OrangeHRM
    // added original password constraint for the new updatePassword function. Please check below.
    ...
    public function updatePassword($userId, $password, $originalPassword) {
        ...
        try {
            ...
            $q = Doctrine_Query::create()
                               ->update('SystemUser')
                               ->set('user_password', '?', $password)
                               ->set('user_password_asp', '?', md5($originalPassword))
                               ->where('id = ?', $userId);
