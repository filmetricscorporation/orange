...
class EmployeeDao extends BaseDao {
    ...
    public function getEmployeeCharacterReferences($empNumber, $characterReferenceCode = null) {
        try {
            $q = Doctrine_Query::create()
                            ...
                            //->leftJoin('es.CharacterReference s')
                            ->where('es.emp_number = ?', $empNumber);

            if (!is_null($characterReferenceCode)) {
                $q->andwhere('es.character_reference_id = ?', $characterReferenceCode);
                return $q->fetchOne();
            }
            $q->orderBy('es.name ASC');
    ...
    public function saveEmployeeCharacterReference(EmployeeCharacterReference $empCharacterReference) {
        ...
        try {
            ...
            $empCharacterReference->save();
            
            return $empCharacterReference;
    ...
    public function deleteEmployeeCharacterReferenceRecords($empNumber, $entriesToDelete = null) {
        ...
        try {
            ...
            $q = Doctrine_Query::create()->delete('EmployeeCharacterReference')
                                         ->where('emp_number = ?', $empNumber);
            
            if (is_array($entriesToDelete) && count($entriesToDelete) > 0) {                
                $q->whereIn('character_reference_id', $entriesToDelete);                
            }
    ...
    // Family Members
    public function getEmployeeFamilyMembers($empNumber) {
        try {
            $q = Doctrine_Query:: create()->from('EmpFamilyMember ef')
                            ->where('ef.emp_number = ?', $empNumber)
                            ->orderBy('ef.name ASC');
            return $q->execute();
        // @codeCoverageIgnoreStart
        } catch (Exception $e) {
    ...
    public function deleteEmployeeFamilyMembers($empNumber, $entriesToDelete = null) {
        
        try {
            
            $q = Doctrine_Query::create()->delete('EmpFamilyMember d')
                                         ->where('emp_number = ?', $empNumber);
     ...
     * @throws DaoException
     */
    public function getEmployeePropertyList($properties, $orderField, $orderBy, $excludeTerminatedEmployees = false, $excludeResignedEmployees = false) {
        ...
        try {
                ...
                if ($excludeResignedEmployees) {
                    $q->andwhere("e.emp_status <> 3");
                }
    ...
    /**
     * Get WorkShift Record(s) for given employee
     * 
     * @version 2.6.11
     * @param int $empNumber Employee number
     * @param int $empSalaryId Employee Basic Salary ID
     ...
     * @todo Exceptions should preserve previous exception
     */
    public function getEmployeeWorkShifts($empNumber, $empWorkShiftId = null) {
        try {
            $q = Doctrine_Query::create()
                ->from('EmployeeWorkShift s')
                ->leftJoin('s.WorkShift c')
                ->where('s.emp_number = ?', $empNumber);

            //if (!is_null($empWorkShiftId)) {
            //    $q->andwhere('s.id = ?', $empWorkShiftId);
            //    return $q->fetchOne();
            //}
    ...
    private function _getEmployeeListQuery(&$select, &$query, array &$bindParams, &$orderBy,
            $sortField = null, $sortOrder = null, array $filters = null, $allow_access_confidential_value = 0) {
            ...
            foreach ($filters as $searchField=>$searchBy ) {
                if (!empty($searchField) && !empty($searchBy)
                    ...
                    $field = self::$searchMapping[$searchField];
                     ...
                     // 20160425
                    if ($allow_access_confidential_value == 0)
                        $conditions[] =  'e.is_confidential = ' . $allow_access_confidential_value;
   ...
   /**
    ...
    public function searchEmployees(EmployeeSearchParameterHolder $parameterHolder) {
        ...
        // 20160425
        $allowAccessConfidentialValue = $parameterHolder->getAllowAccessConfidentialValue();
        ...
        $this->_getEmployeeListQuery($select, $query, $bindParams, $orderBy,
                $sortField, $sortOrder, $filters, $allowAccessConfidentialValue);
